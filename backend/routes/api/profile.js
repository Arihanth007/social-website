...
router.delete('/experience/:exp_id', passport.authenticate('jwt', { session: false }), (req, res) => {
    Profile.findOne({ user: req.user.id })
        .then(profile => {
            const removeIndex = profile.experience.map(item => item.id).indexOf(req.params.exp_id); // Get remove index

            profile.experience.splice(removeIndex, 1); // Splice of out array
...
router.delete('/education/:edu_id', passport.authenticate('jwt', { session: false }), (req, res) => {
    Profile.findOne({ user: req.user.id })
        .then(profile => {
            ...
            profile.education.splice(removeIndex, 1); // Splice of out array
